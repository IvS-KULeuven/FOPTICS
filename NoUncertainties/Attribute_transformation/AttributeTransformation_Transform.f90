...
do i = 1, number_LightCurves ! Loop over the number of light curves (= number of objects)
 do j = 1, NumberFrequencies ! Loop over the number of frequencies
  ...
  Attributes(i,NumberFrequencies+j) = dlog10(LightCurveParameters(i,2+j)) ! extracted frequencies are not subject to any transformation and here we take their log10; Attributes(i,j+NumberFrequencies), with j running through 1 --> NumberFrequencies
...
do i = 1, number_LightCurves ! Loop over the number of light curves
 ...
 do j = 1, ProductFrequenciesHarmonics ! Loop over all frequencies and their harmonics
  ...
  Attributes(i,2*NumberFrequencies+j) = amplitude ! Attributes(i,j), with j running through 2*NumberFrequencies+1 --> 2*NumberFrequencies+ProductFrequenciesHarmonics
...
do i = 1, number_LightCurves ! Loop over the number of light curves
 PowerOdd(i) = PowerOdd(i) + (Attributes(i,2*NumberFrequencies+1))**2.d0 ! Squared amplitude
 do j = 1, ProductFrequenciesHarmonics-1
  ...
  Attributes(i,2*NumberFrequencies+ProductFrequenciesHarmonics+j) = datan2(sintemp,costemp) ! phase mapped into the half-open interval; Attributes(i,j), with j running through 2*NumberFrequencies+ProductFrequenciesHarmonics+1 --> ProductFrequenciesHarmonics-1
! Sum of the odd and even powers
  PowerOdd(i) = PowerOdd(i) + (costemp*Attributes(i,2*NumberFrequencies+1+j))**2.d0
  PowerEven(i) = PowerEven(i) + (sintemp*Attributes(i,2*NumberFrequencies+1+j))**2.d0
 enddo
enddo
