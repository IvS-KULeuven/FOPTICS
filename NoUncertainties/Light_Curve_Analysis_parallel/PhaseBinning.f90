!subroutine PhaseBinning(frequency,BinCentersPerFrequency,FluxBinned,BinMinimumFluxValue,iprint,theta)
subroutine PhaseBinning(frequency,BinCentersPerFrequency,FluxBinned,BinMinimumFluxValue,iprint)

use light_curve
implicit real(8) (a-h,o-z)

!real(8) FluxBinned(NumberBins), PhaseDifference(NumberBins), BinCentersPerFrequency(NumberBins), phase(ntimes), VariancePerBin(NumberBins)
...
do i = 1, NumberBins ! Loop over the number of phase bins
 if(FluxBinned(i) /= 0) then ! When the bin is not empty
  FluxBinned(i) = FluxBinned(i)/real(NumberPointsPerBin(i),8) ! Compute flux in that bin 
...
!if(iprint == 1) then
! VariancePerBin = 0.d0
! do it = 1, ntimes ! Loop over the number of light curve points in time domain
!  phase(it) = dmod((times(it) - times(1))*frequency,1.d0) ! Compute phase
!  PhaseDifference = dabs(BinCenters - phase(it)) ! Compute difference between current phase point and all bin centers to figure out in which particular bin the point will fall
!  PhaseDifferenceMinimum = minval(PhaseDifference); MinimumLocation = minloc(PhaseDifference) ! Find bin center that is closest to the current phase point
!  VariancePerBin(MinimumLocation(1)) = VariancePerBin(MinimumLocation(1)) + (flux_detrended(it) - FluxBinned(MinimumLocation(1)))**2.d0  ! Accumulate sum of squared differences
! enddo

! NumberBinsNonZero = 0; theta = 0.d0
! do i = 1, NumberBins ! Loop over the number of phase bins
!  if(VariancePerBin(i) /= 0) then ! When the bin is not empty
!   VariancePerBin(i) = VariancePerBin(i)/real(NumberPointsPerBin(i)-1,8) ! Compute flux in that bin
!   NumberBinsNonZero = NumberBinsNonZero + 1
!   theta = theta + VariancePerBin(i)*real(NumberPointsPerBin(i)-1,8)
!  endif
! enddo
! theta = theta/real((ntimes - NumberBinsNonZero),8)
! else
! theta = 0.d0
!endif
